...
in time: the local checkout (detached `HEAD` at `d87502a`), the live default
branch (`origin/main` at `a88d9dd`), and the only published release
(`v0.5.2`, tag commit `6c3e8e9`). **The one rule for reading it: every row is
marked VERIFIED (checked directly against the repo, or against a live
`gh`/`git` command) or is flagged as an open question. Where a claim could
not be independently confirmed against this repository, it has been removed
or restated as an open question rather than left standing on its own.**
...
| `login`/`whoami`/`logout` exist, but only in `d87502a` (PR #51, merged to `main`, never tagged) | VERIFIED — `git show d87502a:src/commands/cmd_account.cpp` |
| `usage` does not exist in this checkout at all; it is added only on `origin/main` (PR #60, `6e2c144`), which this checkout is 2 commits behind and which is itself unreleased | VERIFIED — `git show HEAD:src/commands/cmd_usage.cpp` → not found; `git show origin/main:src/commands/cmd_usage.cpp` → 219 new lines |
...
| Statement | Status |
|---|---|
| On `HEAD`, `Login()` takes exactly one URL (`--console-url`/`RCLI_CONSOLE_URL`) and requires the server's `verification_url` to share its **exact origin** — there is no separate web-URL concept in this checkout's code at all (`grep -rn "CONSOLE_WEB_URL" src/` → zero hits) | VERIFIED — `src/commands/cmd_account.cpp:119-145`, `src/account/credentials.cpp:480-484` |
| Consequently: any environment where the console API and the console web app are different origins makes `rcli login` **hard-fail** with "console returned an approval URL outside its origin," before a browser ever opens | VERIFIED by reading the check |
| `AGENTS.md:82-83` documents the wrong default for `RCLI_CONSOLE_URL` ("defaults to `http://localhost:8080`"); the real default (`DefaultConsoleUrl()`) returns `https://console.runanywhere.ai` whenever the env var is unset — an engineer trusting the doc would wrongly believe `rcli login` is inert by default when it targets production | VERIFIED — `AGENTS.md:82-83` vs `src/account/credentials.cpp:37,456-458` |
| **Fixed, but only on `origin/main` (PR #60, `6e2c144`) — not in this checkout, not in any release**: splits the URL into `RCLI_CONSOLE_URL` (API) and `RCLI_CONSOLE_WEB_URL` (approval page); `TrustedBrowserOrigins()` trusts an explicit `RCLI_CONSOLE_WEB_URL` if set, else a hardcoded production allowlist, else the API origin itself for any other (dev) console | VERIFIED — `src/account/credentials.cpp:60-71,522-535` on `origin/main` |
...
| `--no-browser` already works on `HEAD`, printing the code/URL instead of opening a browser | VERIFIED — `cmd_account.cpp:139-146,263-271` |
...
| `rcli usage` exists only on `origin/main` (PR #60): two fixed windows, `"1h"`→`"past 1h"` and `"24h"`→`"past 24h"` (`kRows[]`) | VERIFIED — `git show origin/main:src/commands/cmd_usage.cpp:68` |
...
| `FetchUsage()` calls `GET {origin}/v1/cli/usage?days=<1..365>` with the access token | VERIFIED — `origin/main`, `src/account/console.cpp:753` |
| `rcli whoami`'s README one-liner promises "who you are, and what you have used this month" (`README.md:19`) — **`WhoAmI()` does not do this on either checkout**: it prints only email, session status, and the console URL, never anything from usage data, and PR #60 (which added `usage` as its own command) left `WhoAmI()` untouched | VERIFIED — `src/commands/cmd_account.cpp:215-253` (`HEAD`), `:259-299` (`origin/main`). See RCLI-41 |
...
| **On HEAD, a missing target tool on macOS/Linux produces no user-facing error at all** — `Spawn()` forks and `execvp`s; on failure the child calls `_exit(127)` silently. Only the Windows path (`_spawnvp`) prints `"<tool> is not on PATH"` | VERIFIED — `src/harness/harness.cpp:145-176` |
| **Fixed, but only on `origin/main` (PR #60)**: a new `OnPath()` preflight walks `$PATH` (handling Windows suffixes and the POSIX empty-component-means-cwd rule) before forking, and on a miss prints `"<tool> is not installed on this machine"` plus a per-tool install hint | VERIFIED — `origin/main` diff |
| The unreleased `skills/runanywhere/SKILL.md` (PR #60, 82 lines) implements a "copy-paste terminal skill installs, logs in, drops into a harness" onboarding flow, and explicitly tells the assistant to check "not signed in" first rather than launching a TUI at a brand-new user | VERIFIED |
| Unreleased `install.sh` (PR #60) pulls that skill file from the release tag (not `main`, so an installed assistant's instructions cannot drift under it) and runs `rcli login` automatically post-install unless non-interactive or already signed in | VERIFIED — `install.sh:88-121` on `origin/main` |

### Error handling, tests, docs

| Statement | Status |
|---|---|
| Also fixed only on `origin/main`, implying real bugs on `HEAD`: the local OpenAI-compatible proxy wrote prompt text to a world-readable `/tmp` path; login rejected some legitimately base64url-encoded request codes; a cloud session/model id could be acted on before being verified | VERIFIED — commit messages + diffs on `origin/main` |
| `rcli bench --trials` did not range-check negative integers sensibly on `HEAD`; fixed on `origin/main` | VERIFIED — commit message "fix(bench): range-check --trials as an integer so -n -5 reads sensibly" |
...
| PR #60 adds `tests/test_rcli_harness.cpp` (316 new lines) and substantially expands `test_rcli_account.cpp`, `test_rcli_opencode.cpp`, `test_account_cli.py` — none of this exists in this checkout yet | VERIFIED |
| Tracked docs are exactly `AGENTS.md`, `CLAUDE.md` (symlink), `CONTRIBUTING.md`, `README.md`, `docs/RELEASING.md`, `docs/VERIFICATION.md`, plus 5 mirrored skill files (`.agents/skills/*` and `.claude/skills/*`). No `LAUNCH.md`/`CHECKLIST.md`/`ROADMAP.md` anywhere | VERIFIED — `git ls-files` |
...
| RCLI-6 | Merge this checkout up to `origin/main` (or work directly on `origin/main`) before cutting the release, so the login/usage/harness fixes below actually ship together | This local checkout is 2 commits behind `origin/main`; releasing from a stale branch would omit fixes already merged | CLI maintainer | S | RCLI-5 | repo: `git log HEAD..origin/main` | Open |
| RCLI-7 | Land the `RCLI_CONSOLE_WEB_URL` split-origin fix (PR #60) into the release | This is the exact, verified root cause of the CLI authorization approval flow failing whenever a console's API and web app are on different origins — fixed on `origin/main`, absent from every release | CLI maintainer | (already built, needs release) | RCLI-5 | repo: `src/account/credentials.cpp:60-71,522-535` (origin/main) | Fixed on origin/main, unreleased |
| RCLI-8 | Land `rcli usage` (PR #60) into the release | Currently doesn't exist in any checkout users can install; this is the direct fix for `rcli usage` windows rendering as dashes being visible at all (the dash *content* is a separate, backend-side item, RCLI-9) | CLI maintainer | (already built, needs release) | RCLI-5 | repo: `src/commands/cmd_usage.cpp` (origin/main only) | Fixed on origin/main, unreleased |
...
| ID | Item | Why it is needed | Owner | Size | Blocks | Source | State |
|---|---|---|---|---|---|---|---|
| RCLI-29 | Correct `AGENTS.md`'s wrong `RCLI_CONSOLE_URL` default (already fixed on `origin/main`; needs merging with RCLI-6) | An engineer trusting the current doc would believe `rcli login` is inert by default | — | S | Docs accuracy | repo: `AGENTS.md:82-83` vs `credentials.cpp:37,456-458` | Fixed on origin/main, unreleased |
...
|---|---|---|
| **Dev/prod console origin mismatch** — console API and browser-approval page on different hosts | The CLI should discover and trust the right origin automatically, or fail with a message that tells the operator exactly which env var to set | On `HEAD`: hard, unconditional fail — "console returned an approval URL outside its origin" — with **no mention of any fix**, because no such fix exists in this checkout. On `origin/main` (unreleased): fails the same way unless the operator already knows to set `RCLI_CONSOLE_WEB_URL`; there is still no autodiscovery |
| **A hostname the console currently hands out for the approval page changes** | Sign-in should keep working through any purely infrastructural redeploy | Production trust is a **hardcoded two-string allowlist** (the custom console domain plus one other literal hostname). If that second hostname changes, production sign-in breaks until someone edits `credentials.cpp` and cuts a new release. **Not fixed even on `origin/main`.** The code comment's own 2026-09-04 measurement is a mitigating fact for right now: both hostnames currently serve the same deployment, meaning they are one deployment behind two names today, not two independently-trusted origins — the risk is real but latent until that changes |
...
| **Usage spend exceeds ~$2,147 on Windows** | Numbers should render correctly at any spend level | Fixed on `origin/main` (moved to `int64` counters); the bug (32-bit `long` overflow) is real on `HEAD` and any release, since `usage` doesn't exist pre-fix anyway — the risk only manifests once RCLI-8 ships |
| **Missing coding-tool binary at harness launch time** | A clear, actionable message (an install command, then "run this again") | On Windows: message printed. On **macOS/Linux — the platforms this product actually ships on**: `execvp` fails and the child calls `_exit(127)` with zero output anywhere. Fixed on `origin/main` via a `PATH`-walking preflight, unreleased |
| **Tool removed from `PATH` between the preflight check and the actual `fork`/`exec`** | Same clear message, even in the race window | Not handled — `OnPath()` (the origin/main fix) is a check-then-use pattern with an inherent, if narrow, gap |
| **Local OpenAI-compatible proxy on a shared/multi-user machine** | Prompt text should never be readable by another local user | On `HEAD`: prompts are written to a **world-readable `/tmp` path** — a real local information-disclosure risk. Fixed on `origin/main` (0600 permissions, no longer under `/tmp`), unreleased |
| **A CI test standing in for the Windows credential store** | The test should exercise the real DPAPI-encrypted read/write path | On `HEAD`'s test suite (per PR #60's fix list): a test hand-wrote a **plaintext** session file to stand in for the store, so the encrypted path itself could be broken while this test still passed. Fixed on `origin/main`, unreleased |
