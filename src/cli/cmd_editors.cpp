...
int Serve(const std::string& model, bool verbose) {
    ...
    out::Line("ANTHROPIC_BASE_URL=" + shim.base_url);
    out::Line("ANTHROPIC_AUTH_TOKEN=" + shim.auth_token);
    out::Status("serving " + model + "; press Ctrl-C to stop");
...
int Restore(const Editor& editor) {
    ...
    if (editor.wiring == Wiring::JetBrainsProvider) {
        ...
        out::Status(std::string(editor.id) + " no longer points at a local model");
        return 0;
    }
    ...
    out::Status(std::string(editor.id) + " is back on Anthropic; restart it to pick that up");
    return 0;
}
...
int Run(const Editor& editor, const std::string& model,
    ...
    if (is_bundle) {
        ...
        if (bundle.empty()) {
            out::Error(std::string(editor.bundle) + " is not installed");
            return 1;
        }
#else
        out::Error(std::string(editor.id) + " is a macOS application");
        return 1;
#endif
    }
    ...
    if (editor.wiring == Wiring::JetBrainsProvider) {
        ...
        out::Status(std::string(editor.id) + " will talk to " + model + " through " + reachable);
        QuitBundle(bundle);
        ...
        if (status == 0) {
            ...
            out::Status("serving " + model + " until " + std::string(editor.id) + " quits");
    ...
    out::Status(std::string(editor.id) + " will talk to " + model + " through " +
                shim.base_url);
    if (advertised != model) {
        out::Status("advertised to the app as " + advertised + "; the picker shows " + model);
    }
...
void RegisterEditors(CLI::App& app, Options& options) {
    for (const Editor& editor : kEditors) {
        ...
        command->callback([&options, &editor, model, rest, serve, restore] {
            if (*restore) {
                options.status = Restore(editor);
                return;
            }
            options.status = *serve ? Serve(*model, options.verbose)
