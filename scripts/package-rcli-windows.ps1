...
if ([string]::IsNullOrWhiteSpace($Version)) {
    ...
    if ($cm -match 'project\(rcli VERSION ([0-9.]+)') { $Version = $Matches[1] }
}
$Version = $Version.TrimStart("v")
if ([string]::IsNullOrWhiteSpace($Version)) { throw "cannot resolve RCLI version" }

$Binary = @(
    (Join-Path $BuildDir "rcli.exe"),
    (Join-Path $BuildDir "Release\rcli.exe"),
    (Join-Path $BuildDir "RelWithDebInfo\rcli.exe")
) | Where-Object { Test-Path $_ } | Select-Object -First 1
if (-not $Binary) {
    $Binary = Get-ChildItem -Path $BuildDir -Filter "rcli.exe" -File -Recurse |
        Select-Object -ExpandProperty FullName -First 1
}
if (-not $Binary) { throw "rcli.exe was not found under $BuildDir" }

if ([string]::IsNullOrWhiteSpace($KitDir)) {
    $KitDir = $env:RCLI_SDK_KIT
}
if ([string]::IsNullOrWhiteSpace($KitDir)) {
...
$Zip = Join-Path $DistDir "rcli-$Version-$Platform.zip"
