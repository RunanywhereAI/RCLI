/**
 * @file test_wally_telemetry_live.cpp
 ...
 * POST (the same recipe as bootstrap's wally_telemetry_http_callback) so we can
 ...
 *     ./test_wally_telemetry_live --live
 */
...
// wally_telemetry_http_callback (all-public rac_http_* / rac_auth_* APIs).
struct LiveState {
...
int main(int argc, char** argv) {
    ...
    opts.quiet = true;
    wally::Bootstrapped env;
    const rac_result_t brc = wally::bootstrap(opts, &env);
    CHECK(brc == RAC_SUCCESS, "bootstrap succeeded");
    if (brc != RAC_SUCCESS) {
        return 1;
    }

    rac_telemetry_manager_t* mgr = wally::active_telemetry_manager();
    ...
    // LLM
    {
        ...
        g->set_model_id("wally-live-test");
    ...
    // Embeddings
    {
        ...
        c->set_model_id("wally-live-test");
        c->set_input_count(1);
    ...
    // RAG
    {
        ...
        (*ev.mutable_properties())["embedding_model"] = "wally-live-test";
    ...
    // LoRA (failure path — rides the LLM component, modality overridden to lora)
    {
        v1::SDKEvent ev;
        envelope(&ev, v1::SDK_COMPONENT_LLM);
        (*ev.mutable_properties())["adapter_id"] = "wally-live-test";
